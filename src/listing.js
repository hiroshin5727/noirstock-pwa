...
export function archiveSalesChannel(data, channelId) {
  const channel = data.salesChannels.find(c => c.id === channelId);
...
export function restoreSalesChannel(data, channelId) {
  const channel = data.salesChannels.find(c => c.id === channelId);
  if (!channel) return null;
  channel.active = true;
  channel.updatedAt = new Date().toISOString();
...
export function updateListingRecord(data, listingId, patch) {
  const listing = data.listingRecords.find(l => l.id === listingId);
