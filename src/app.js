...
import { channelTypeText, listingStatusText, addSalesChannel, addListingRecord, addEventRecord, markListingSold, activeListingsForInventory } from './listing.js';
import { channelStats, eventStats, staleListings, listingLeadTimeStats } from './analytics.js';
...
function render() {
  app.innerHTML = `
    <div class="app">
      <header class="header">
        <div>
          <div class="brand">NoirStock <span class="badge">v6.4.1</span></div>
...
function channelCard(c) {
  return `<div class="item-card"><div class="row"><div><div class="name">${escapeHtml(c.name)}</div><div class="muted">${channelTypeText(c.type)} / 手数料 ${c.feeRate || 0}% / 固定 ${yen(c.fixedFee)}</div></div><span class="pill ${c.active ? 'green' : 'amber'}">${c.active ? '利用中' : '停止'}</span></div></div>`;
}
...
function analysisView(d) {
  ...
  return `<main class="grid">
    <div class="row wrap"><div><h1 class="screen-title">分析</h1><div class="muted">出品先・イベント・ハンドメイドを分析</div></div><select style="max-width:180px" data-action="change-year">${yearOptions()}</select></div>
    <section class="kpi grid">
      <div class="panel"><div class="muted">年間売上</div><div class="money">${yen(tax.salesTotal)}</div></div>
      <div class="panel"><div class="muted">実利益</div><div class="money">${yen(tax.netProfit)}</div><div>${pct(tax.margin)}</div></div>
      <div class="panel"><div class="muted">平均出品→販売</div><div class="money">${lead.avg.toFixed(1)}日</div><div>${lead.count}件</div></div>
    </section>
    <section class="panel">
      <h2>出品先別 売上・利益</h2>
      <div class="card-list">${chStats.length ? chStats.map(channelStatCard).join('') : '<div class="muted">販売場所付きの販売データがまだありません。</div>'}</div>
    </section>
    <section class="panel">
      <h2>イベント別収支</h2>
    ...
    <section class="panel">
      <h2>売れ残り・長期出品</h2>
      <div class="card-list">${stale.length ? stale.map(staleCard).join('') : '<div class="muted">出品中データはありません。</div>'}</div>
    </section>
    <section class="panel">
      <h2>申告モード</h2>
      <div class="finance">
      ...
      <button class="btn primary" data-action="mark-inventory-checked" style="margin-top:12px">${state.year}年 棚卸確認済みにする</button>
    </section>
    <section class="panel">
      <h2>経費分類</h2>
...
function openSettingsModal() {
  ...
  const m = modal('設定 / バックアップ', `<div class="grid"><div class="section-actions"><button class="btn primary" data-set="export">JSONバックアップ</button><label class="btn ghost">JSON Import<input type="file" accept=".json,application/json" data-set="import" style="display:none"></label><button class="btn ghost" data-set="csv">帳簿CSV</button><button class="btn danger" data-set="reset">初期化</button></div><div class="panel"><b>データ検査</b><div class="muted">警告 ${v.warnings.length}件 / エラー ${v.errors.length}件</div></div></div>`);
  m.querySelector('[data-set="export"]').onclick = () => downloadText(`noirstock_backup_v6_4_1_${Date.now()}.json`, JSON.stringify(data(), null, 2));
  ...
  m.querySelector('[data-set="import"]').onchange = async e => {
    const file = e.target.files[0]; if (!file) return;
    try {
      const parsed = JSON.parse(await readFileAsText(file));
      const n = normalizeToV64(parsed), r = validateDataset(n);
      ...
      if (ok) { createAutoBackup('before-import'); replaceDataset(n); m.remove(); toast('Importしました'); render(); }
    } catch (err) { toast(`Import失敗: ${err.message}`); }
  };
}
